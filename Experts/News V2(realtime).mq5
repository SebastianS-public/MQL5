...
void OnTick(){
   ...
   if(newCandle || !executed || TimeCurrent() > nextNewsTime + 5*60){
   ...
   if(TimeCurrent() > nextNewsTime-1*60 && OrderSelect(buyPos) && OrderSelect(sellPos) && !PositionSelectByTicket(buyPos) && !PositionSelectByTicket(sellPos)){
   ...
   if(newCandleM5 && TimeCurrent() > nextNewsTime + 10*60){
...
void executeBuyManagement(){
   ...
   if(OrderSelect(sellPos) && TimeCurrent() < nextNewsTime + 5*60){
...
void executeSellManagement(){
   ...
   if(OrderSelect(buyPos) && TimeCurrent() < nextNewsTime + 5*60){
      if(OrderGetDouble(ORDER_PRICE_OPEN) != iOpen(_Symbol,PERIOD_M5,0)){
