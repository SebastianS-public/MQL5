...
void executeBuy(){
   ...
   if(rsiLongCheck){
      if(SymbolInfoDouble(_Symbol,SYMBOL_ASK) < high - 5 * SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE)){
         trade.BuyStop(lots,high,_Symbol,buyStopLoss,buyTakeProfit,ORDER_TIME_GTC);
         buyPos = trade.ResultOrder();
      }
      else{
         trade.Buy(lots,_Symbol,0,buyStopLoss,buyTakeProfit);
         buyPos = trade.ResultOrder();
      }
   }
}
...
void executeSell(){
   ...
   if(rsiShortCheck){
      if(SymbolInfoDouble(_Symbol,SYMBOL_BID) > low + 5 * SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE)){
         trade.SellStop(lots,low,_Symbol,sellStopLoss,sellTakeProfit,ORDER_TIME_GTC);
         sellPos = trade.ResultOrder();
      }
      else{
         trade.Sell(lots,_Symbol,0,sellStopLoss,sellTakeProfit);
         sellPos = trade.ResultOrder();
      }
   }
}
