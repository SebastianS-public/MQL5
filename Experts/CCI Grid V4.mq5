...
input double CCITriggerConstant = 1.0;
...
void OnTick(){
   ...
   if(newCandle){
      ...
      if(atrValue < 0){
         CCITriggerLevel = CCITriggerLevelStart + (atrValue / SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE) * CCITriggerConstant);
      }
      if(atrValue > 0){
         CCITriggerLevel = CCITriggerLevelStart + (atrValue / SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE) * CCITriggerConstant);
      }
...
double calcATRValue(){
   ...
   int atrHandleLong = getATRHandleLong();
...
bool getATRCondition(){
   int atrHandle = getATRHandleLong();
   double atrArray[];
...
int getATRHandleLong(){
   return iATR(_Symbol,Timeframe,ATRPeriod);
}
